...
interface AppointmentsState {
  ...
  isTimeSlotAvailable: (date: string, time: string, excludeId?: string) => boolean
}
  ...
  persist(
    (set, get) => ({
      ...
      getAppointmentsByTime: (time) => {
        return get().appointments.filter((apt) => apt.time === time && apt.status === 'approved')
      },

      getTodayAppointments: () => {
        const today = new Date().toISOString().split('T')[0]
        return get().appointments.filter((apt) => apt.date === today && apt.status === 'approved').length
      },
      ...
      getAverageAppointmentDuration: () => {
        const approvedAppointments = get().appointments.filter(apt => apt.status === 'approved')
        if (approvedAppointments.length === 0) return 45
        
        const totalDuration = approvedAppointments.reduce((sum, apt) => sum + apt.duration, 0)
